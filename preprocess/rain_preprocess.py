...
for i in range(474,475):
    print("i", i)
    img = cv2.imread(r'.\dataset_vehicle\2 rain_highway\t1\in000%d.jpg' %i)
    Val = random.randint(200, 400)
    noise = get_noise(img, value=Val)
    Len = random.randint(10,70)
    Ang = random.randint(-30,30)
    W = random.randint(0,1)
    rain = rain_blur(noise, length=Len, angle=Ang, w=2*W+1)
    rain_result = alpha_rain(rain, img, beta=0.6)  # 方法一，透明度赋值
    # add_rain(rain,img)    #方法二,加权后有玻璃外的效果
    cv2.imwrite(r".\dataset_vehicle\2 rain_highway\t1\in000%d.jpg" %i, rain_result)

# img = cv2.imread(r'.\dataset_vehicle\rain_highway\t1\in001000.jpg')
#
# Val = random.randint(100,400)
# noise = get_noise(img,value=Val)
# Len = random.randint(5,70)
# Ang = random.randint(-30,30)
# W = random.randint(0,1)
# rain = rain_blur(noise,length=5,angle=Ang,w=2*W+1)
